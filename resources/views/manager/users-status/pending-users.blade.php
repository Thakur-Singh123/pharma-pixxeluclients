                    ...
                    <div class="col-md-12">
                        <div class="card">
                             <div class="card-header d-flex justify-content-between align-items-center">
                                    ...
                                    <form method="GET" action="{{ route('manager.pending.users') }}" class="m-0 d-flex align-items-center" style="gap: 10px;">
                                        <input type="date"
                                            name="joining_date"
                                            class="form-control"
                                        ...
                                        >
                                        <select name="user_type" class="form-control" onchange="if(this.value==''){ window.location='{{ route('manager.pending.users') }}'; } else { this.form.submit(); }">
                                            ...
                                            <div class="col-sm-12">
                                                <table id="basic-datatables" class="display table table-striped table-hover dataTable" role="grid" aria-describedby="basic-datatables_info">
                                                    ...
                                                    <tbody>
                                                        @php $count = 1 @endphp
                                                        <!--Get users-->
                                                        @forelse ($pending_users as $pending)
                                                        <tr role="row">
                                                            <td class="sorting_1">{{ $count++ }}.</td>
                                                            <td>{{ $pending->employee_code }}</td>
                                                            <td>{{ $pending->name }}</td>
                                                            <td>{{ $pending->email }}</td>
                                                            <td>{{ $pending->phone }}</td>
                                                            <td>{{ $pending->city }}</td>
                                                            ...
                                                            <td> 
                                                                {{  $pending->user_type == 'purchase_manager' ? 'Purchase Manager' : 
                                                                ($pending->user_type == 'counsellor' ? 'Counsellor' : 
                                                                ucfirst($pending->user_type)) }}
                                                            </td>
                                                            <td>
                                                            ...
                                                            <td>
                                                                <span class="status-badge 
                                                                    ...
                                                                    {{ $pending->status == 'Suspend' ? 'status-suspend' : '' }} 
                                                                    {{ $pending->status == 'Active' ? 'status-active' : '' }} 
                                                                    {{ $pending->status == 'Approved' ? 'status-approved' : '' }}">
                                                ...
                                                {{ $pending_users->appends(request()->query())->links('pagination::bootstrap-5') }}
                                            </div>
                                        </div>
                                    </div>
                                </div>
...
function handleFilterChange(select) {
    if (select.value === "") {
        window.location.href = "{{ route('manager.pending.users') }}";
    } else {
        select.form.submit();
    }
}
